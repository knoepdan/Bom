CREATE PROCEDURE dbo.[DeletePathXXProc]
	@pathId AS INT NULL,
...
BEGIN
	...
	BEGIN TRY
		...
		IF @currentLevel <= 1 AND EXISTS(SELECT PathId FROM dbo.[Path] WHERE NodePath.IsDescendantOf(@currentPath) = 1 AND [Level] = (@currentLevel + 1))
		BEGIN
